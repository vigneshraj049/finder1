...
import { getAllRealProperties, RealProperty, RealMediaItem } from "@/lib/realApi";
import { formatDate, truncate } from "@/lib/format";

export const Route = createFileRoute("/admin/all-listings")({
  head: () => ({
    meta: [
      { title: "All Listings — Scrapehouse Admin" },
      { name: "description", content: "View all scraped marketplace listings from the database." },
...
function AdminAllListings() {
  ...
  const results = useMemo(() => {
    let data = (propertyQuery.data ?? []) as RealProperty[];
    if (filterDate) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold tracking-tight text-foreground">Global Listings</h1>
        <p className="text-sm text-muted-foreground">
          View all aggregated marketplace properties across all your previous searches.
        </p>
      </div>
      ...
      <Card className="shadow-card border-border/80">
        <CardHeader className="flex flex-row items-center justify-between gap-4 border-b border-border/50 pb-4">
          <div className="flex items-center gap-3">
            <CardTitle className="text-base font-semibold">All Properties Database</CardTitle>
            {results.length > 0 && (
              <Badge variant="outline" className="font-normal text-xs bg-muted/40">
                {results.length} Total {results.length === 1 ? "Listing" : "Listings"}
              </Badge>
            )}
          </div>
        ...
        <CardContent className="pt-6">
          {propertyQuery.isLoading ? (
            ...
            <div className="rounded-lg border border-destructive/20 bg-destructive/5 p-6 text-sm text-destructive">
              Failed to load all properties from the database.
            </div>
          ) : results.length === 0 ? (
